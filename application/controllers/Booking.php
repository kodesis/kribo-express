    ...
    public function store_booking()
    {
        ...
        if (is_array($origins)) {
            for ($i = 0; $i < count($origins); $i++) {
                // Cek apakah ada nilai maksimum no_urut pada hari ini
                if (!$max_num['max']) {
                    $bilangan = 1; // Jika belum ada, mulai dari 1
                } else {
                    $bilangan = $max_num['max'] + 1; // Jika sudah ada, tambah 1
                }

                // Format no_urut menjadi 4 digit
                $no_urut = sprintf("%04d", $bilangan);
                // Membuat no_booking dengan format kode + no_urut
                $no_booking = $kode . $no_urut;

                // Ambil data dari form untuk setiap baris booking
                ...
                $data[] = [
                    ...
                    'gudang_tujuan' => trim($this->input->post('lokasi_gudang')),
                ...
                // Update nilai no_urut untuk booking berikutnya
