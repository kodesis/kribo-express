...
class Master extends Authenticated_Controller
{
					...
					for ($i = 2; $i <= count($sheetData); $i++) {
						...
						if (!isset($groups[$key])) {
							$groups[$key] = [
								...
								'harga_modal'     => ($category === 'DOMESTIC') ? $this->_parse_indo_number($sheetData[$i]['E'] ?? 0) : 0,
								'harga_jual'    => ($category === 'DOMESTIC') ? $this->_parse_indo_number($sheetData[$i]['F'] ?? 0) : 0,
						...
						$groups[$key]['tiers'][] = [
							'harga_modal'  => $this->_parse_indo_number($sheetData[$i]['E'] ?? 0),
							'harga_jual' => $this->_parse_indo_number($sheetData[$i]['F'] ?? 0),
	...
	public function confirm_import_pricelist()
	{
		...
		foreach ($groups as $g) {
			...
			$master_data = [
				...
				'harga_modal'     => ($g['is_tiered'] == 0) ? $g['tiers']['harga_modal'] : 0,
				'harga_jual'    => ($g['is_tiered'] == 0) ? $g['tiers']['harga_jual'] : 0,
			];
	...
	public function download_template_pricelist()
	{
		...
		// Header Lengkap A-I
		$csv_content = "Origin,Destination,Service_ID,Category,harga_modal,harga_jual,Min_Weight_Charge,Tier_Min,Tier_Max\n";
		...
		force_download('Template_Pricelist_Kribo_V2.csv', $csv_content);
	}
	...
	public function ajax_cek_harga()
	{
		...
		if ($pricelist) {
			$harga_jual = $pricelist->harga_jual;
			$harga_modal  = $pricelist->harga_modal;
			...
			if ($pricelist->is_tiered == 1) {
				...
				if ($tier) {
					$harga_jual = $tier->harga_jual;
					$harga_modal  = $tier->harga_modal;
				} else {
					...
					if ($last_tier) {
						$harga_jual = $last_tier->harga_jual;
						$harga_modal  = $last_tier->harga_modal;
					}
				}
			}
