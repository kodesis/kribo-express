...
class M_Shipment extends CI_Model
{
	...
	public function generate_no_resi()
	{
		...
		$date_prefix = date('ym');
		$prefix = 'KRX' . $date_prefix; // Output: SMC260331
	...
	public function get_stats($agent_id = NULL)
	{
        ...
        SUM(total_amount) as total_tagihan -- Ini adalah Harga Kribo yang harus mereka bayar
    ")->get('shipments')->row();
	}
	...
	public function get_superadmin_stats()
	{
        ...
        -- TOTAL PROFIT (Selisih Harga Jual - Harga Modal)
        SUM(CASE WHEN MONTH(created_at) = MONTH(CURDATE()) 
            AND YEAR(created_at) = YEAR(CURDATE()) 
            THEN margin_amount ELSE 0 END) as profit_kribo_bulan_ini
		")->get('shipments')->row();
	}
