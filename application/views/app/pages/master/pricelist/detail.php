						...
						<div class="card-body">
							<div class="datagrid">
								<div class="datagrid-item <?= hide_if_not_admin() ?>">
									<div class="datagrid-title">Harga Modal</div>
									<div class="datagrid-content text-danger">Rp <?= number_format($pricelist->harga_modal) ?></div>
								</div>
								<div class="datagrid-item">
									<div class="datagrid-title">Harga Jual</div>
									<div class="datagrid-content text-success h3 mb-0">Rp <?= number_format($pricelist->harga_jual) ?></div>
								</div>
								<div class="datagrid-item <?= hide_if_not_admin() ?>">
									<div class="datagrid-title">Potensi Margin</div>
									<div class="datagrid-content fw-bold">Rp <?= number_format($pricelist->harga_jual - $pricelist->harga_modal) ?></div>
								</div>
							</div>
						</div>
					<?php else: ?>
						<div class="table-responsive">
							<table class="table table-vcenter card-table">
								<thead>
									<tr>
										<th>Range Berat</th>
										<th class="text-end <?= hide_if_not_admin() ?>">Modal</th>
								...
								<tbody>
									<?php foreach ($tiers as $t): ?>
										<tr>
											...
											<td class="text-end text-danger <?= hide_if_not_admin() ?>">Rp <?= number_format($t->harga_modal) ?></td>
											<td class="text-end text-success fw-bold">Rp <?= number_format($t->harga_jual) ?></td>
											<td class="text-end fw-bold <?= hide_if_not_admin() ?>">Rp <?= number_format($t->harga_jual - $t->harga_modal) ?></td>
										</tr>
									<?php endforeach; ?>
								</tbody>
							</table>
